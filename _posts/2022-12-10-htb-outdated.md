---
title: "HTB - Outdated"
excerpt: "Hack The Box Machines • Windows • Medium"
layout: "htb_machine"
htb:
  machine:
    name: "Outdated"
    url: "https://app.hackthebox.com/machines/Outdated"
  avatar: "/assets/images/htb-machines/avatars/outdated.png"
  difficulty: "Medium"
  os: "Windows"
  author:
    - name: "ctrlzero"
      url: "https://app.hackthebox.com/users/168546"
---

## Synopsis

Outdated is a medium Windows machine provided by Hack The Box that features an Active Directory domain controller that is vulnerable to `CVE-2022-30190`. Successful exploitation of this gets us a shell as a user called `btables`. This user holds a special privilege over the user `sflowers` that allows us to conduct a _shadow credentials_ attack and authenticate as them. The user `sflowers` is part of a group that can create and approve WSUS updates which we can abuse to execute commands and get a shell as `NT Authority\SYSTEM`.
...
```bash
$ echo 'rhost=10.10.11.175' > .env
$ echo 'lhost=10.10.14.3' >> .env # Change me
$ . ./.env
$ rustscan -a $rhost -b 2500 -u 3000 -- -Pn -sV -oN scan.txt
```

Some services mentioned in the scan results are `25/SMTP`, `53/DNS`, `389/LDAP`, `445/SMB`, and `5985/WinRM`. The presence of these services indicate that this machine is probably an Active Directory domain controller.
...
It turns out we can actually connect to this share without credentials! There is only one readable file in the share called `NOC_Reminder.pdf`.
...
![](/assets/images/htb-machines/outdated/document.png)
...
After doing some research into each CVE mentioned in the document, we find out that one of the vulnerabilities is well documented and has public exploits. This exploit is labeled as `CVE-2022-30190`, but is more commonly refered to as _Follina_.

## Follina

_Follina_ is a remote code execution vulnerability that can be exploited when a user or application opens a malicious link using the MSDT URL protocol. Since the document we downloaded earlier suggested that this flaw was not patched, it would be worth our time to investigate.
...
We can verify that the url is opened by setting up a web server and sending the URL to `itsupport@outdated.htb`. To do this, we can use a CLI tool called [swaks](https://github.com/jetmore/swaks).
...
**Warning:** If you are not receiving any requests after a few minutes, go ahead and reset the machine because this feature can be inconsistent at times.
{: .notice--info}
...
At this point, we can try to exploit CVE-2022-30190 using a custom exploit script from [here](https://gist.github.com/bryanmcnulty/a02d96eb10b3beb4ea35115993b1981a) and [Villain](https://github.com/t3l3machus/Villain).
...
![](/assets/images/htb-machines/outdated/bloodhound.png)
...
![](/assets/images/htb-machines/outdated/bloodhound2.png)
